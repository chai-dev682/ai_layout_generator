...
class OpenAIDeedParser:
    ...
    def _parse_multiple_tracts(self, deed_text: str, settings: ProjectSettings) -> DeedParseResult:
        ...
        try:
            ...
            # Convert settings to dict if it's a Pydantic model to avoid validation issues
            ...
            result = DeedParseResult.create_flexible(
                tracts=tracts,
            ...
            # Try with minimal data and dict settings
            try:
                # Ensure settings is a dict for minimal result
                if hasattr(settings, 'model_dump'):
                    minimal_settings = settings.model_dump()
                elif hasattr(settings, 'dict'):
                    minimal_settings = settings.dict()
                else:
                ...
                # Create a basic dict-based result as last resort
                try:
                    result = DeedParseResult.create_flexible(
                        tracts=[],
                        settings={},  # Empty dict as absolute fallback
                        calls=[],
                        geometry=None,
                        original_text=deed_text or ""
                    )
                    logger.info("Created emergency fallback DeedParseResult")
                except:
                    raise minimal_error
