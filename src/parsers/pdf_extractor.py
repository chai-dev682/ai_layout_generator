...
Cross-platform compatible for Windows and Linux (Streamlit Cloud).
"""
import os
import io
import base64
...
from ..utils.platform_config import platform_config
...
class PDFExtractor:
    ...
    def __init__(self, 
        ...
        logger.info(f"Platform: {platform_config.platform}")
        logger.info(f"Streamlit Cloud: {platform_config.is_streamlit_cloud}")
    ...
    def _configure_platform(self):
        """Configure platform-specific settings"""
        # Configure Tesseract
        self._tesseract_available = platform_config.configure_pytesseract()
        
        if not self._tesseract_available:
            logger.warning("Tesseract OCR not properly configured - image-based PDFs may not work")
        
        # Log platform information
        logger.info(f"Platform: {platform_config.platform}")
        logger.info(f"Tesseract command: {platform_config.tesseract_cmd}")
        logger.info(f"Is Streamlit Cloud: {platform_config.is_streamlit_cloud}")
    ...
    def _extract_with_pdfminer_tesseract(self, pdf_file_path: str, metadata: Dict[str, Any]) -> PDFExtractionResult:
        ...
        try:
            ...
            if not extracted_text.strip() or len(extracted_text.strip()) < 100:
                if not self._tesseract_available:
                    raise Exception("PDF appears to be image-based but Tesseract OCR is not available")
